/* bxcan.c — bxCAN app driver. PB8=CAN_RX, PB9=CAN_TX, 250kbps @ 8MHz. */
#include "bxcan.h"
...
#define CAN_MCR_LBKM   (1U<<30) /* actually in BTR */
...
void bxcan_app_init(uint32_t bps) {
    (void)bps; /* Fixed at 250kbps for now */
    RCC_APB2ENR |= (1U<<0)|(1U<<3);  /* AFIO + GPIOB */
    RCC_APB1ENR |= (1U<<25);         /* CAN */
    AFIO_MAPR = (AFIO_MAPR & ~(3U<<13)) | (2U<<13); /* remap to PB8/PB9 */
    GPIOB->CRH = (GPIOB->CRH & ~(0xFFU)) | (0x4U)|(0xBU<<4); /* PB8=float-in, PB9=AF-PP */

    CAN1->MCR &= ~CAN_MCR_SLEEP;
    _enter_init();
    CAN1->MCR |= CAN_MCR_NART;  /* no auto-retry = OSM */
    CAN1->BTR = (0U<<28)|(0U<<24)|(2U<<20)|(11U<<16)|(1U<<0); /* 250k@8MHz */
...
int bxcan_app_tx(uint32_t id, const uint8_t *data, uint8_t len, int extended) {
    ...
    if (!t) return -1;
    uint32_t tir;
    if (extended) tir = (id<<3) | CAN_TIR_IDE;
    else          tir = (id<<21);
    CAN1->TXB[0].TIR  = tir;
    CAN1->TXB[0].TDTR = len;
    uint32_t dL=0,dH=0;
